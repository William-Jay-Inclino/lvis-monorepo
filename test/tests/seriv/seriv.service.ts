...
export const goto_create_seriv_page = async(payload: { page: Page, url: string }) => {
    ...
    await x.click({ page, test_id: 'seriv-menu' })
    await x.click({ page, test_id: 'create-seriv' })

    await expect(page).toHaveURL(`${url}/warehouse/seriv/create`, { timeout: 5000 });
    
}

export const goto_seriv_view_page = async(payload: { page: Page, url: string, seriv_number: string }) => {

    const { page, url, seriv_number } = payload
    ...
    await x.custom_select({
        page,
        test_id: 'search-seriv-number',
        value: seriv_number
    })
    
    await x.click({ page, test_id: 'search' })
    await x.click({ page, test_id: `view-details-${seriv_number}` })

    const dynamicUrlPattern = new RegExp(`${url}/warehouse/seriv/view/[a-zA-Z0-9-]+`)
    await expect(page).toHaveURL(dynamicUrlPattern, { timeout: 5000 });
    
}

export const verify_items_to_be_transacted = async(payload: { page: Page, url: string, seriv_number: string }) => {
    ...
    for(let item of items) {
        ...
        await expect(page.getByTestId(`seriv-${ seriv_number }`)).toBeVisible();
...
export const create_seriv = async(payload: { page: Page, data: SerivData, url: string, item_code: string }): Promise<{ seriv_number: string }> => {
    ...
    const seriv_number = await x.getText({ page, test_id: 'seriv-number' })
