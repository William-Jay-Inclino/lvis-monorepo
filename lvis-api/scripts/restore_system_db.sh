...
if [ -z "$1" ]; then
  ...
  echo "Usage: ./restore_system_db.sh <backup_filename>"
  exit 1
fi

BACKUP_FILE="$1"
BACKUP_PATH="$BACKUP_SYSTEM_DIR/$BACKUP_FILE"
...
echo "$(date +'%Y-%m-%d %H:%M:%S') - Initializing system database restore..."
...
docker exec -i "$DB_CONTAINER_NAME" psql -U "$DB_USER" -d "$DB_SYSTEM_NAME" -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"
docker exec -i "$DB_CONTAINER_NAME" pg_restore -U "$DB_USER" -d "$DB_SYSTEM_NAME" -v /tmp/backup.dump
...
echo "$(date +'%Y-%m-%d %H:%M:%S') - System database restore process completed."
