...
LOG_DIR=$(dirname "$BACKUP_SYSTEM_LOG")
if [ ! -d "$LOG_DIR" ]; then
  mkdir -p "$LOG_DIR"
...
if [ ! -d "$BACKUP_SYSTEM_DIR" ]; then
  echo "$(date +'%Y-%m-%d %H:%M:%S') - Backup directory does not exist. Creating directory..." >> "$BACKUP_SYSTEM_LOG"
...
while [ "$(ls -1 "$BACKUP_SYSTEM_DIR" | wc -l)" -ge "$MAX_BACKUPS" ]; do
  ...
  echo "$(date +'%Y-%m-%d %H:%M:%S') - Removed old backup file: $OLDEST_BACKUP" >> "$BACKUP_SYSTEM_LOG"
done
...
while true; do   # Continuously show spinner until the backup process finishes
  ...
  docker exec "$DB_CONTAINER_NAME" pg_dump -U "$DB_USER" -d "$DB_SYSTEM_NAME" -Fc -E utf-8 > "$BACKUP_FILE" && break
done

# Check if the backup was successful
if [ $? -eq 0 ] && [ -s "$BACKUP_FILE" ]; then
  echo "$(date +'%Y-%m-%d %H:%M:%S') - Backup successful: $BACKUP_FILE" >> "$BACKUP_SYSTEM_LOG"
  echo "Backup completed successfully!"   # Display success message
else
  echo "$(date +'%Y-%m-%d %H:%M:%S') - Backup failed." >> "$BACKUP_SYSTEM_LOG"
