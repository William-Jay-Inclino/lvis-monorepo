...
LOG_FILE="/home/jay/Documents/projects/leyeco/lvis-monorepo/backup/logs/truncate_all_tables_in_powerserve.log"
...
echo "$(date +'%Y-%m-%d %H:%M:%S') - Initializing truncation of all tables in $DB_POWERSERVE_NAME..." >> "$LOG_FILE"

# Ensure we have a valid connection
echo "$(date +'%Y-%m-%d %H:%M:%S') - Checking database connection..." >> "$LOG_FILE"
docker exec -i "$DB_CONTAINER_NAME" psql -U "$DB_USER" -d "$DB_POWERSERVE_NAME" -c "\q" &>/dev/null
if [ $? -ne 0 ]; then
...
docker exec -i "$DB_CONTAINER_NAME" psql -U "$DB_USER" -d "$DB_POWERSERVE_NAME" -c "
DO \$\$
DECLARE
   r RECORD;
BEGIN
...
OUTPUT=$(docker exec -i "$DB_CONTAINER_NAME" psql -U "$DB_USER" -d "$DB_POWERSERVE_NAME" -c "
DO \$\$
DECLARE
   r RECORD;
BEGIN
...
echo "All tables in $DB_POWERSERVE_NAME have been truncated."
