...
import { MCRT as M, MCRTItem } from 'apps/warehouse/prisma/generated/client';
...
export class McrtPdfService {
    ...
    async generatePdf(mcrt: MCRT, metadata: { ip_address: string, device_info: any, authUser: AuthUser }) {
        ...
        const approvers = await Promise.all(mcrt.mcrt_approvers.map(async (i) => {
            i.approver = await this.getEmployee(i.approver_id, authUser);
            return i;
        }));

        const [returned_by] = await Promise.all([
            this.getEmployee(mcrt.returned_by_id, authUser),
        ])
        ...
        <div class="content">
            ...
            <div style="flex-grow: 1;">
                ...
                <div style="display: flex; justify-content: flex-end;">
                    ...
                    <div>
                        <table style="font-size: 8pt; width: 200px;">
                            ...
                            <tr>
                                <td>Date:</td>
                                <td style="border-bottom: 1px solid black; font-weight: bold; text-align: center"> ${formatDate(mcrt.mcrt_date)} </td>
                            </tr>
                        </table>
                    </div>
                ...
                <br />
                <div class="heading" style="text-align: center;"> MATERIAL CHARGE RETURN TICKET </div>
                <br />
                <br />
                ...
                <table border="0" style="width: 100%; font-size: 9pt; border-collapse: collapse; ">
                    <tbody>
                        <tr>
                            ...
                            <td style="width: 78%; padding-top: 10px; border-bottom: 1px solid black; font-weight: bold; white-space: pre-line; padding-left: 10px;">${ mcrt.note }</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <table border="0" class="item-table">
                    ...
                    <tbody>
                        ${mcrt.mcrt_items.map((mcrt_item, index) => `
                        <tr style="border: 1px solid black;">
                            <td align="center" style="padding-top: 10px; padding-bottom: 10px; vertical-align: middle;">${index + 1}</td>
                            <td style="padding-top: 10px; padding-bottom: 10px; vertical-align: middle; white-space: nowrap;">${mcrt_item.item.code}</td>
                            <td style="white-space: pre-line; padding-top: 10px; padding-bottom: 10px; vertical-align: middle;">${mcrt_item.item.description} ${ mcrt_item.item.project_item ? `(${ mcrt_item.item.project_item.project.name })` : '' }</td>
                            <td style="padding-top: 10px; padding-bottom: 10px; vertical-align: middle; white-space: nowrap;" align="center">${mcrt_item.quantity}</td>
                            <td style="padding-top: 10px; padding-bottom: 10px; vertical-align: middle; white-space: nowrap;" align="center">${mcrt_item.item.unit.name}</td>
                            <td style="padding-top: 10px; padding-bottom: 10px; vertical-align: middle; white-space: nowrap;" align="center">${formatToPhpCurrency(mcrt_item.price)}</td>
                            <td style="padding-top: 10px; padding-bottom: 10px; vertical-align: middle; white-space: nowrap;" align="center">${formatToPhpCurrency(mcrt_item.price * mcrt_item.quantity)}</td>
                        </tr>
                    `).join('')}
                    </tbody>
                    <tfoot>
                ...
                <div style="display: flex; flex-wrap: wrap;">
                    ...
                    <div style="padding: 10px; width: 45%; padding-top: 30px;"> 
                        ...
                        <table border="0" style="width: 100%; font-size: 8pt;">
                            ...
                            <tr>
                                <td> ${formatDate(mcrt.mcrt_date)} </td>
                            </tr>
                            <tr>
        ...
        const pdfArrayBuffer = await page.pdf({
            ...
            <div style="border-top: solid 1px #bbb; width: 100%; font-size: 9px;
                ...
                <div style="position: absolute; left: 5px; top: 5px;">
                    Note: System generated report | Created by: <b>${ mcrt.created_by }</b> | Printed by: <b>${authUser.user.username}</b> | 
    ...
    async findMcrt(id: string): Promise<M> {
        const item = await this.prisma.mCRT.findUnique({
            include: {
                mcrt_items: {
                    select: {
                        quantity: true,
    ...
    private get_grand_total_price(mcrt_items: MCRTItem[]) {
    
        let total_price = 0

        for(let mcrt_item of mcrt_items) {
            total_price += mcrt_item.price * mcrt_item.quantity
        }
