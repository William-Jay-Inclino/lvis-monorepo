import type { ChangeApproverResponse } from "./seriv.types";
...
export async function changeApprover(id: string, approver_id: string): Promise<ChangeApproverResponse> {
    ...
    try {
        ...
        if (response.data && response.data.data && response.data.data.changeSerivApprover) {
            return {
                success: true,
                msg: 'Approver changed successfully!',
                data: response.data.data.changeSerivApprover
            };
        }
