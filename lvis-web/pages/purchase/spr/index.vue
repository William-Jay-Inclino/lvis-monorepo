        ...
        <div class="card-body">
            ...
            <div v-if="!isLoadingPage && authUser">
                ...
                <div class="row pt-3">
                    <div class="col-lg-4 col-md-6 col-sm-12">
                        <div class="mb-3">
                            ...
                            <client-only>
                                <v-select data-testid="search-spr-number" @search="handleSearchSprNumber" :options="store.search_filters.sprs" label="spr_number" v-model="store.search_filters.spr"></v-select>
                            </client-only>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12">
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button data-testid="create-spr" v-if="canCreate(authUser, 'canManageSPR')" @click="onClickAdd" class="btn btn-primary float-end">
                        <client-only>
                            ...
                            <div class="col">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        <thead>
                                            <tr>
                                                <th class="bg-secondary text-white no-wrap">SPR Number</th>
                                        ...
                                        <tbody>
                                            <tr @click="store.selected_row_indx = indx" :class="{'table-warning': indx === store.selected_row_indx}" v-for="i, indx in store.items">
                                                <td class="text-muted align-middle no-wrap"> {{ i.spr_number }} </td>
                ...
                getFullname(i.canvass.requested_by!.firstname,
                    i.canvass.requested_by!.middlename, i.canvass.requested_by!.lastname) : 'N/A' }}
                                                ...
                                                <td class="align-middle text-center no-wrap">
                                                    <button :data-testid="`view-details-${ i.spr_number }`" @click="onClickViewDetails(i.id)" class="btn btn-light btn-sm" :class="{ 'text-primary': canViewDetails(authUser, 'canManageSPR') }"
...
import { useSprStore } from '~/composables/purchase/spr/spr.store';
...
onMounted(async () => {
    ...
    const { canvasses, sprs, employees } = await sprApi.fetchDataInSearchFilters()
    store.set_search_filters({ canvasses, sprs, employees })
...
async function search() {
    ...
    // find by SPR NUMBER
    if (store.search_filters.spr) {
        const response = await sprApi.findBySprNumber(store.search_filters.spr.spr_number)
        isSearching.value = false
        if (response) {
    ...
    if (store.search_filters.canvass) {
        const response = await sprApi.findByRcNumber(store.search_filters.canvass.rc_number)
        isSearching.value = false
        if (response) {
            store.set_searched_results({ items: [response] })
            return
