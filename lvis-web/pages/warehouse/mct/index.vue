        ...
        <div class="card-body">
            ...
            <div v-if="!isLoadingPage && authUser">
                ...
                <div class="row pt-3">
                    <div class="col-lg-3 col-md-6 col-sm-12">
                        <div class="mb-3">
                            ...
                            <client-only>
                                <v-select data-testid="search-mct-number" @search="handleSearchMctNumber" :options="store.mcts" label="mct_number" v-model="store.search_filters.mct"></v-select>
                            </client-only>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12">
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button data-testid="create-mct" v-if="canCreate(authUser, 'canManageMCT')" @click="onClickAdd" class="btn btn-primary float-end">
                        <client-only>
                            ...
                            <div class="col">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        <thead>
                                            <tr>
                                                <th class="bg-secondary text-white no-wrap">MCT Number</th>
                                        ...
                                        <tbody>
                                            <tr @click="store.selected_row_indx = indx" :class="{'table-warning': indx === store.selected_row_indx}" v-for="i, indx in store.items">
                                                <td class="text-muted align-middle no-wrap"> {{ i.mct_number }} </td>
                                                ...
                                                <td class="text-muted align-middle no-wrap"> {{ formatDate(i.mct_date) }}
                                                </td>
                                                ...
                                                <td class="align-middle text-center no-wrap">
                                                    <button :data-testid="`view-details-${ i.mct_number }`" @click="onClickViewDetails(i.id)" class="btn btn-light btn-sm" :class="{ 'text-primary': canViewDetails(authUser, 'canManageMCT') }"
...
import { useMctStore } from '~/composables/warehouse/mct/mct.store';
...
onMounted(async () => {
    ...
    const { mcts, employees } = await mctApi.fetchDataInSearchFilters()
    store.set_search_filters({ mcts, employees })
...
async function search() {
    ...
    if (store.search_filters.mct) {
        const response = await mctApi.findByMctNumber(store.search_filters.mct.mct_number)
        isSearching.value = false
        if (response) {
...
async function searchMctNumbers(input: string, loading: (status: boolean) => void) {
    ...
    try {
        ...
        store.set_search_filters({ mcts: response })
    } catch (error) {
