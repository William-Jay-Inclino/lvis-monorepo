    ...
    <div class="card">
        <div class="card-body">
            ...
            <div v-if="!isLoadingPage">
                ...
                <h2 class="text-warning">Unit</h2>
        
                <hr>
        
                <div class="row">
                    <div class="col">
                        <button v-if="canCreate(authUser, 'canManageUnit', SERVICES.SYSTEM)" @click="onClickCreate"
                ...
                <div class="row justify-content-center pt-5">
                    <div class="col-lg-8">
                        <div class="input-group mb-3">
                            <input type="text" class="form-control" placeholder="search for unit name..." v-model="searchValue">
                        </div>
                    </div>
                </div>
                            ...
                            <div class="col">
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        <thead>
                                            <tr>
                                                <th class="bg-secondary text-white">Unit Name</th>
                                                ...
                                                <td class="text-center">
                                                    <div class="d-inline-flex">
                                                        <button :disabled="!canDelete(authUser, 'canManageUnit')"
                                                            ...
                                                            <client-only>
                                                                <font-awesome-icon :icon="['fas', 'trash']" :class="{ 'text-danger': canDelete(authUser, 'canManageUnit') }"/>
                                                            </client-only>
                                                        </button>
                                                        <button :disabled="!canEdit(authUser, 'canManageUnit')"
                                                            @click="onClickEdit(i.id)" class="btn btn-sm btn-light">
                                                            <client-only>
                                                                <font-awesome-icon :icon="['fas', 'edit']" :class="{ 'text-primary': canEdit(authUser, 'canManageUnit') }" />
                                                            </client-only>
                                                        </button>
                                                    </div>
                                                </td>
...
const onClickCreate = () => router.push('/warehouse/unit/create')
const onClickEdit = (id: string) => router.push('/warehouse/unit/' + id)
