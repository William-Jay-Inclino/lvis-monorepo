            ...
            <div v-if="!isLoadingPage && authUser" class="row justify-content-center">
                ...
                i.approver!.middlename, i.approver!.lastname) }} </td>
                                                <td v-if="!isBlankStatus(item.status, i.status)"
                                                    class="text-muted text-center align-middle no-wrap">
                                                    <div
                            ...
                            <div class="col">
                                <div class="d-flex justify-content-center flex-wrap gap-2">
                                    <nuxt-link v-if="canSearch(authUser, 'canManageSERIV')"
                                        class="btn btn-secondary" :class="{'w-100 w-md-auto': isMobile}" to="/warehouse/SERIV">
                                        <client-only>
                                            <font-awesome-icon :icon="['fas', 'search']" />
                                        </client-only> 
                                        Search SERIV
                                    </nuxt-link>
                                    <button v-if="item.status === APPROVAL_STATUS.APPROVED && canPrint(authUser, 'canManageSERIV')" @click="onClickPrint()" class="btn btn-danger" :class="{'w-100 w-md-auto': isMobile}">
                                        <client-only>
                                    ...
                                    <button
                                        ...
                                        class="btn btn-danger" :class="{'w-100 w-md-auto': isMobile}"
                                    >
                                        <client-only>
                                    ...
                                    <template v-if="!item.cancelled_at">
                                        <button v-if="isAdminOrOwner(item.created_by, authUser) && item.status === APPROVAL_STATUS.PENDING"
                                            class="btn btn-warning" :class="{'w-100 w-md-auto': isMobile}" @click="onCancelSERIV()">
                                            <client-only>
                                        ...
                                        <button v-if="canCreate(authUser, 'canManageSERIV')"
                                            class="btn btn-primary" :class="{'w-100 w-md-auto': isMobile}" @click="onClickAdd">
                                            <client-only>
