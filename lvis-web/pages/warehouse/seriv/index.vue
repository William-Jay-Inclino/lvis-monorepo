                ...
                <div class="row pt-3">
                    <div class="col-lg-3 col-md-6 col-sm-12">
                        <div class="mb-3">
                            ...
                            <client-only>
                                <v-select data-testid="search-seriv-number" @search="handleSearchSerivNumber" :options="store.serivs" label="seriv_number" v-model="store.search_filters.seriv"></v-select>
                            </client-only>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12">
                            ...
                            <div class="col">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        <thead>
                                            <tr>
                                                <th class="bg-secondary text-white no-wrap">SERIV Number</th>
                                        ...
                                        <tbody>
                                            <tr @click="store.selected_row_indx = indx" :class="{'table-warning': indx === store.selected_row_indx}" v-for="i, indx in store.items">
                                                <td class="text-muted align-middle no-wrap"> {{ i.seriv_number }} </td>
...
import { useSerivStore } from '~/composables/warehouse/seriv/seriv.store';
...
onMounted(async () => {
    ...
    const { serivs, employees } = await serivApi.fetchDataInSearchFilters()
    store.set_search_filters({ serivs, employees })
...
async function search() {
    ...
    // find by RV NUMBER
    if (store.search_filters.seriv) {
        const response = await serivApi.findBySerivNumber(store.search_filters.seriv.seriv_number)
        isSearching.value = false
        if (response) {
...
async function searchSerivNumbers(input: string, loading: (status: boolean) => void) {
    ...
    try {
        ...
        store.set_search_filters({ serivs: response })
    } catch (error) {
