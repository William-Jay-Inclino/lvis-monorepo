    ...
    <div class="card">
        <div class="card-body">
            ...
            <div v-if="!isLoadingPage && authUser">
                <h2 class="text-warning">Search MRV</h2>
                ...
                <div class="row pt-3">
                    <div class="col-lg-3 col-md-6 col-sm-12">
                        <div class="mb-3">
                            ...
                            <client-only>
                                <v-select data-testid="search-mrv-number" @search="handleSearchMrvNumber" :options="store.mrvs" label="mrv_number" v-model="store.search_filters.mrv"></v-select>
                            </client-only>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6 col-sm-12">
...
import { useMrvStore } from '~/composables/warehouse/mrv/mrv.store';

definePageMeta({
    name: ROUTES.MRV_INDEX,
...
onMounted(async () => {
    ...
    const { mrvs, employees } = await mrvApi.fetchDataInSearchFilters()
    store.set_search_filters({ mrvs, employees })
...
async function changePage(page: number) {
    ...
    const { data, currentPage, totalItems, totalPages } = await mrvApi.findAll({
        page,
...
async function search() {
    ...
    if (store.search_filters.mrv) {
        const response = await mrvApi.findByMrvNumber(store.search_filters.mrv.mrv_number)
        isSearching.value = false
        if (response) {
...
async function searchMrvNumbers(input: string, loading: (status: boolean) => void) {
    ...
    try {
        const response = await mrvApi.fetchMrvNumbers(input);
        console.log('response', response);
        store.set_search_filters({ mrvs: response })
    } catch (error) {
...
const onClickViewDetails = (id: string) => router.push('/warehouse/mrv/view/' + id)
