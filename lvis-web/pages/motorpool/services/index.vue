    ...
    <div class="card">
        <div class="card-body">
            ...
            <div v-if="!isLoadingPage">
                ...
                <h2 class="text-warning">PMS Services</h2>
        
                <hr>
        
                <div class="row">
                    <div class="col">
                        <button v-if="canCreate(authUser, 'canManageVehicleService')" @click="onClickCreate"
                ...
                <div class="row justify-content-center pt-5">
                    <div class="col-lg-8">
                        <div class="input-group mb-3">
                            <input type="text" class="form-control" placeholder="search for service name..." v-model="searchValue">
                        </div>
                    </div>
                </div>
                            ...
                            <div class="col">
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        <thead>
                                            <tr>
                                                <th class="bg-secondary text-white">Service Name</th>
                                            ...
                                            <tr v-for="i in filteredItems">
                                                ...
                                                <td class="text-center">
                                                    <div class="d-inline-flex">
                                                        <button :disabled="!canDelete(authUser, 'canManageVehicleService')"
                                                            @click="onClickDelete(i.id)" class="btn btn-sm btn-light me-3">
                                                            <client-only>
                                                                <font-awesome-icon :icon="['fas', 'trash']" :class="{ 'text-danger': canDelete(authUser, 'canManageVehicleService') }"/>
                                                            </client-only>
                                                        </button>
                                                        <button :disabled="!canEdit(authUser, 'canManageVehicleService')"
                                                            @click="onClickEdit(i.id)" class="btn btn-sm btn-light">
                                                            <client-only>
                                                                <font-awesome-icon :icon="['fas', 'edit']" :class="{ 'text-primary': canEdit(authUser, 'canManageVehicleService') }" />
                                                            </client-only>
                                                        </button>
                                                    </div>
                                                </td>
...
const items = ref<VehicleService[]>([])
...
const onClickCreate = () => router.push('/motorpool/services/create')
const onClickEdit = (id: string) => router.push('/motorpool/services/' + id)
