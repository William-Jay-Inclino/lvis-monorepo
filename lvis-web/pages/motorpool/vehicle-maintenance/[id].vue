    ...
    <div>
            ...
            <div v-if="!isLoadingPage && authUser" class="card">
                <div class="card-body">
                    ...
                    <h2 class="text-warning mb-4">Update PMS Record</h2>
                    <hr>
                            ...
                            <div class="row mt-4">
                                <div class="col">
                                    <div class="d-flex justify-content-between pt-3">
                                        <div>
                                            <nuxt-link class="btn btn-secondary" :to="`/motorpool/vehicle-maintenance/view/${vmData.id}`">
                                                <client-only>
...
import { type CreateVehicleMaintenanceDetail, type UpdateVehicleMaintenance } from '~/composables/motorpool/vehicle-maintenance/vehicle-maintenance.types';
...
definePageMeta({
    name: ROUTES.VEHICLE_MAINTENANCE_UPDATE,
...
// FORM DATA
const vmData = ref<UpdateVehicleMaintenance>({
    id: '',
    vehicle: null,
    service_center: null,
    service_date: '',
...
onMounted(async () => {
    ...
    let response = await vmApi.fetchFormDataInUpdate(route.params.id as string)

    if (!response.vehicle_maintenance) {
    ...
    const services = response.services.map(i => {
        return {
            service: i,
            note: '',
            isChecked: false,
    ...
    const vmDataServices = response.vehicle_maintenance.services.map(i => {
        return {
            service: i.service,
            note: i.note,
            isChecked: true,
    ...
    response.vehicle_maintenance.vehicle['label'] = response.vehicle_maintenance.vehicle.vehicle_number + ' ' + response.vehicle_maintenance.vehicle.name

    vmData.value = {
        id: response.vehicle_maintenance.id,
        vehicle: response.vehicle_maintenance.vehicle,
        service_center: response.vehicle_maintenance.service_center,
        service_date: formatToValidHtmlDate(response.vehicle_maintenance.service_date),
        next_service_date: formatToValidHtmlDate(response.vehicle_maintenance.next_service_date),
        service_mileage: response.vehicle_maintenance.service_mileage,
        next_service_mileage: response.vehicle_maintenance.next_service_mileage,
        cost: response.vehicle_maintenance.cost,
        remarks: response.vehicle_maintenance.remarks,
        performed_by: response.vehicle_maintenance.performed_by,
        services: mergeServices(services, vmDataServices)
    }
...
async function update() {
    ...
    const data: UpdateVehicleMaintenance = {
        id: vmData.value.id,
    ...
    const response = await vmApi.update(vmData.value.id, data)
...
function mergeServices(services: CreateVehicleMaintenanceDetail[], vmDataServices: CreateVehicleMaintenanceDetail[]) {
    
    const merged = [...services, ...vmDataServices];

    const unique = merged.reduce((acc, current) => {
        const existing = acc.find(item => item.service.id === current.service.id);

        if (existing) {
            existing.isChecked = existing.isChecked || current.isChecked;
        } else {
            acc.push(current);
        }

        return acc;
    }, [] as CreateVehicleMaintenanceDetail[]);
