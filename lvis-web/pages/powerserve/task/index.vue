        ...
        <div class="card mt-3">
            <div class="card-body">
                <div v-if="!isLoadingPage && authUser">
                    <h2 class="text-warning">Search Task</h2>
                    ...
                    <div class="row pt-3">
                        <div class="col-lg-4 col-md-6 col-sm-12">
                            <div class="mb-3">
                                ...
                                <client-only>
                                    <v-select @search="handleSearchRefNumber" :options="store.tasks" label="rc_number" v-model="store.search_filters.task"></v-select>
                                </client-only>
                            </div>
                        </div>
                        <div class="col-lg-4 col-md-6 col-sm-12">
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <!-- <button data-testid="create-task" v-if="canCreate(authUser, 'canManageTask')" @click="onClickAdd"
                                        ...
                                        <table class="table table-hover">
                                            ...
                                            <tbody>
                                                <tr @click="store.selected_row_indx = indx" :class="{'table-warning': indx === store.selected_row_indx}" v-for="i, indx in store.items">
                                                    ...
                                                    <td class="text-muted align-middle no-wrap"> [TBA] </td>
                                                    ...
                                                    <td class="align-middle text-center no-wrap">
                                                        <button @click="onClickViewDetails(i.id)" class="btn btn-light btn-sm" :class="{ 'text-primary': canViewDetails(authUser, 'canManageTask') }"
                                                            :disabled="!canViewDetails(authUser, 'canManageTask')">
                                                            <client-only>
...
import * as api from '~/composables/powerserve/task/task.api'
...
onMounted(async () => {
    ...
    const { task_statuses } = await api.task_index_init()
...
async function search() {
    ...
    if (store.search_filters.task) {

        const response = await api.findByRefNumber(store.search_filters.task.ref_number)
        isSearching.value = false
...
async function searchRefNumbers(input: string, loading: (status: boolean) => void) {
    ...
    try {
        ...
        console.log('response', response);
        store.set_search_filters({ tasks: response })
    } catch (error) {
...
const onClickViewDetails = (id: number) => router.push('/powerserve/task/view/' + id)
const onClickAdd = () => router.push('/powerserve/task/create')
